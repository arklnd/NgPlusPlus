...
describe('updatePackageWithDependencies', function () {
    ...
    it('should invoke dumbResolverHandler with HYUI9 Angular dependencies', async function () {
        ...
        try {
            ...
            if (resultText.includes('✅ Successfully updated dependencies')) {
                ...
                // Check some key dependencies were updated with major version 20
                ...
                expect(semver.major(semver.coerce(coreVersion!)!)).to.equal(20);
                ...
                expect(semver.major(semver.coerce(cliVersion!)!)).to.equal(20);
    ...
    it('should invoke dumbResolverHandler with HYUI8 Angular dependencies', async function () {
        ...
        // Arrange - Copy asset files to test directory
        const assetsDir = path.join(__dirname, 'assets');
        const sourcePackageJsonPath = path.join(assetsDir, '/HYUI8/package.json');
        const sourcePackageLockPath = path.join(assetsDir, '/HYUI8/package-lock.json');
        const originalGitPath = path.join(path.resolve(assetsDir), 'git-git');
    ...
    it('should invoke dumbResolverHandler with NGE Angular dependencies', async function () {
        // Increase timeout for npm operations
        this.timeout(3600000); // 60 minutes

        // Arrange - Copy asset files to test directory
        const assetsDir = path.join(__dirname, 'assets');
        const sourcePackageJsonPath = path.join(assetsDir, '/NGE/package.json');
        const sourcePackageLockPath = path.join(assetsDir, '/NGE/package-lock.json');
        const originalGitPath = path.join(path.resolve(assetsDir), 'git-git');
