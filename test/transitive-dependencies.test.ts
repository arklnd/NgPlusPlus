...
import { updateTransitiveDependencies } from '../src/utils/transitive-dependencies.utils.js';
import { readPackageJson } from '../src/utils/package-json.utils.js';
...
describe('updateTransitiveDependencies', function () {
    ...
    it('should handle invalid version specifications gracefully', async function () {
        ...
        const updates: PackageUpdate[] = [
            { name: "@angular/core", version: "invalid-version" }
        ];
        ...
        const hasInvalidVersionError = results.some(result => 
            result.includes('❌') && result.includes('Invalid version specification')
        );
    ...
    it('should handle network errors gracefully', async function () {
        ...
        const updates: PackageUpdate[] = [
            { name: "non-existent-package-xyz-123", version: "^1.0.0" }
        ];
        ...
        const hasNetworkError = results.some(result => 
            result.includes('❌') && result.includes('Failed to process')
        );
    ...
    it('should handle packages with peer dependencies', async function () {
        ...
        const hasPeerDepResults = results.some(result => 
            result.includes('@angular/core') || result.includes('@angular/common')
        );
    ...
    it('should handle mixed dependency types (dev and prod)', async function () {
        ...
        const mentionsDependencies = results.some(result => 
            result.includes('@angular') || result.includes('dependencies')
        );
